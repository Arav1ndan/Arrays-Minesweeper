...
namespace UI {
    ...
    void MainMenuManager::registerButtonCallbacks() {
        play_button->registerCallbackFunction([this](UIElement::MouseButtonType buttonType)
            {
                playButtonCallback(buttonType);
            }
        );
        quit_button->registerCallbackFunction([this](UIElement::MouseButtonType buttonType)
            {
